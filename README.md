# Container Image Sync (Tencent TCR + optional self-hosted registries)

通过 GitHub Actions 将 Docker Hub、GHCR 或其他 OCI Registry 中的镜像同步到目标 Registry（默认腾讯云 TCR，也可指向自建 Harbor 或其它 Registry）。无需自建服务器，支持完整保留多架构镜像，也支持只同步指定平台。

默认只把每个镜像推送到 TCR（`PUSH_TARGETS=tcr`），需要时可额外写入自建 Harbor 等其它 Registry。

## 功能

- 在 `images.txt` 中每行写一个镜像，提交后自动同步本次新增或修改的行
- 默认推送到 TCR，可用 `PUSH_TARGETS=tcr,harbor` 一次写入多个 Registry
...
### 2. 自建 Harbor 或其它 Registry（可选）

只有当 `PUSH_TARGETS` 里包含对应名称时才需要准备：
...
| 名称 | 示例 | 说明 |
| --- | --- | --- |
| `PUSH_TARGETS` | `tcr` | 要推送的 Registry 列表，顺序即优先级，默认为 `tcr`；需要同时推自建 Registry 时写 `tcr,harbor` |
| `TCR_REGISTRY` | `demo.tencentcloudcr.com` | TCR 域名，只填域名，不含协议和仓库路径 |
| `TCR_NAMESPACE` | `mirror` | 未写目标镜像时 TCR 使用的命名空间，默认 `mirror` |
| `HARBOR_REGISTRY` | `harbor.example.com` | 仅在 `PUSH_TARGETS` 含 `harbor` 时需要的自建 Harbor 域名 |
| `HARBOR_NAMESPACE` | `library` | 仅在 `PUSH_TARGETS` 含 `harbor` 时需要的 Harbor 项目名 |
...
| `TCR_USERNAME` | 是 | TCR 访问凭证用户名，仅在 `PUSH_TARGETS` 含 `tcr` 时需要 |
| `TCR_PASSWORD` | 是 | TCR 访问凭证密码，仅在 `PUSH_TARGETS` 含 `tcr` 时需要 |
| `HARBOR_USERNAME` | 否 | Harbor 机器人账号或用户名，仅在 `PUSH_TARGETS` 含 `harbor` 时需要 |
| `HARBOR_PASSWORD` | 否 | Harbor 密码或机器人 Token，仅在 `PUSH_TARGETS` 含 `harbor` 时需要 |
| `SOURCE_USERNAME` | 否 | 私有源 Registry 用户名 |
| `SOURCE_PASSWORD` | 否 | 私有源 Registry 密码或 Token |

`PUSH_TARGETS` 中列出的每个 Registry 都必须同时配置对应的 `*_REGISTRY`、`*_USERNAME`、`*_PASSWORD`；未写显式目标镜像时还需要 `*_NAMESPACE`。默认只推送到 TCR，因此 Harbor 的变量和密钥可以暂时不配置；等自建 Registry 就绪后，把 `PUSH_TARGETS` 改为 `tcr,harbor` 并补齐 Harbor 配置即可。
...
假设默认 `PUSH_TARGETS=tcr`、`TCR_NAMESPACE=mirror`，上面三行同步后的目标分别是：

```text
demo.tencentcloudcr.com/mirror/nginx:1.27
demo.tencentcloudcr.com/mirror/app:v1
demo.tencentcloudcr.com/mirror/nginx:1.27
```

如果还设置了 `PUSH_TARGETS=tcr,harbor` 和 `HARBOR_NAMESPACE=library`，则每个镜像会再多推一份到 `harbor.example.com/library/...`。
...
2. 选择 `Sync container image to Tencent TCR and self-hosted registries`。
...
`PUSH_TARGETS` 里的其它 Registry 同理，例如含 `harbor` 时还会推送到 `${HARBOR_REGISTRY}/${target_image}`。
...
- 为 CI/CD 创建独立的 TCR 服务级账号与自建 Registry 机器人账号，仅授予目标命名空间或仓库的推送权限。
